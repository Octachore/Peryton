...
{
    public class HighOrderMarkovGenerator<T>
    {
        private readonly IDictionary<T[], MarkovState<T>> _possibleInitialStates;

        public HighOrderMarkovGenerator(IDictionary<T[], MarkovState<T>> possibleInitialStates)
        {
        ...
        public IEnumerable<T> Generate(int max, T[] initialValues)
        {
            var visitor = new MarkovChainVisitor<T>(_possibleInitialStates[initialValues]);
